...
export default function EventsScreen({navigation}){
  ...
  useEffect(()=>{
    AsyncStorage.getItem("access_token").then(tok=>
      getEvents(tok).then(d=>setEvents(d.results||d)).catch(console.error)
    )
  },[])

  return(
    <View style={s.container}>
      <FlatList data={events} keyExtractor={i=>String(i.id)}
        renderItem={({item})=>(
          <TouchableOpacity style={s.card} onPress={()=>navigation.navigate("Detail",{event:item})}>
            <Text style={s.name}>{item.title}</Text>
            <Text style={s.info}>{item.city} — {new Date(item.start_at).toLocaleDateString("fr-FR")}</Text>
            {item.end_at&&<Text style={s.info}>Fin : {new Date(item.end_at).toLocaleDateString("fr-FR")}</Text>}
          </TouchableOpacity>
        )}
